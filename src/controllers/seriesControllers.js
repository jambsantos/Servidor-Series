...
const writeSerie = ()=> {
   return fs.writeFile("./src/models/series.json",JSON.stringify(series),"utf8",function(err){
        if (err){
...
const updateSeries = (req,res)=>{
    ...
    if (serieIndex >=0) {
        ...
        writeSerie()
         ...
         res.status(404).send({message:"Série não encontrado para ser atualizada!"})
    }
    res.status(200).send(series)
}
